...
# --- 3. 기본 정보 입력 ---
st.title("⚙️ 조립 1라인 스마트 작업일보 (버전 4.5)")
...
if 'rows' not in st.session_state:
    ...
    for i, (s, e) in enumerate(TIME_SLOTS_BASE):
        st.session_state.rows.append({
            ...
            "m": 60.0 if i not in [3, 6] else 50.0, "is_split": False, "fixed_target": 0
        })
    ...
    if c[10].button("➕", key=f"add_{rid}"):
        ...
        if new_start_dt < row['end']:
            ...
            st.session_state.rows.insert(idx + 1, {
                "id": st.session_state.next_id, "display_time": f"{new_start_dt.strftime('%H:%M')}~{row['end'].strftime('%H:%M')}",
                "start": new_start_dt, "end": row['end'], "m": rem_m, "is_split": True, 
...
# --- 5. 2. 실적 분석 ---
st.markdown("<div class='section-title'>📋 2. 실적 분석 및 품질 추적</div>", unsafe_allow_html=True)
summary_data = []
for r in st.session_state.rows:
    p_name = st.session_state.get(f"p_{r['id']}", "선택")
    if p_name != "선택":
        summary_data.append({"p_name": p_name, "actual": st.session_state.get(f"a_{r['id']}", 0), "defect": st.session_state.get(f"dq_{r['id']}", 0)})
...
if summary_data:
    ...
    for idx, row in df_sum.iterrows():
        ...
        ac[1].write(f"{row['actual'] - row['defect']} EA")
        ac[4].text_input("LOT", key=f"lot_sum_{idx}", label_visibility="collapsed")

# --- 6. 3. 전동 드라이버 토크 측정 기록 (이사님 지시: 1개만 입력해도 전송 가능하게 수정) ---
...
valid_torque_any = False # 1개라도 입력되었는가?
ng_found = False # NG가 있는가?
...
for k in range(1, 6):
    ...
    if t_val:
        try:
            val = float(t_val)
            valid_torque_any = True # 값이 들어왔으므로 True
            if val >= 15: tr[2].markdown('<div class="ok-label">OK</div>', unsafe_allow_html=True)
                ...
                ng_found = True # 15 미만이므로 NG
        except: ng_found = True

# --- 7. 데이터 최종 전송 (버전 4.5: 유연한 전송 로직) ---
if st.button("📊 오늘의 실적 데이터 최종 전송 및 저장", type="primary", use_container_width=True):
    if not valid_torque_any:
        st.error("⚠️ 오류: 최소 1개 이상의 토크 실측값을 입력해야 합니다!")
    elif ng_found:
        st.error("⚠️ 오류: 입력된 토크 값 중 NG($15Kgf/cm$ 미만)가 있습니다. 확인 후 다시 시도하세요.")
    else:
        try:
            ...
            for r in st.session_state.rows:
                ...
                if p != "선택":
                    final_data.append({
                        ...
                        "Time_Slot": r['display_time'], "Invested_Min": st.session_state.get(f"m_{rid}", 0),
                        "Item_Name": p, "Target_UPH": round((PRODUCT_DB.get(p, 0) / 60) * st.session_state.get(f"m_{rid}", 0), 1),
                        "Actual_Qty": st.session_state.get(f"a_{rid}", 0), "Defect_Type": st.session_state.get(f"dt_{rid}", "없음"),
                        "Defect_Qty": st.session_state.get(f"dq_{rid}", 0), "Downtime_Reas": st.session_state.get(f"dr_{rid}", "없음"),
                        "Downtime_Min": st.session_state.get(f"dm_{rid}", 0), "Torque_Value": " / ".join(torque_list),
                        "Material_Check": st.session_state.get(f"lot_sum_0", "")
                    })
            if final_data:
                df = conn.read(worksheet="sheet1")
                updated = pd.concat([df, pd.DataFrame(final_data)], ignore_index=True)
                conn.update(worksheet="sheet1", data=updated)
                st.success("✅ 구글 시트로 성공적으로 전송되었습니다!"); st.balloons()
