...
if st.button("🚀 데이터 최종 전송 (v7.5.7)", type="primary", use_container_width=True):
    try:
        ...
        for r in st.session_state.rows:
            ...
            if p != "선택":
                final_data.append({
                    "Timestamp": ts, "Work_Date": work_date.strftime("%Y-%m-%d"), 
                    "Worker_Name": FIXED_WORKER_NAME, # 고정된 이름 전송
        ...
        st.success("✅ [버전 7.5.7] 구글 시트 전송 완료!"); st.balloons()
