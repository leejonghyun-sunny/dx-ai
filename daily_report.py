...
# --- 1. 페이지 설정 및 UI 고정 (다크테마 & 화살표 제거 & 점멸 삭제 CSS) ---
st.set_page_config(page_title="스마트작업일보 조립1라인 (v7.6.0)", layout="wide")
...
<style>
    ...
    /* [수정] 비상 호출 시각화 알람 (점멸 기능 삭제, 정적 고정 바 적용) */
    .emergency-alarm-static { 
...
# [타이틀 강제 고정]
st.title("스마트작업일보 조립1라인 (v7.6.0)")

# [1. 비상 호출 시각화 알람 (이사님 지시: 점멸 삭제, 문구 고정)]
if st.session_state.issue_state:
    st.markdown(f"<div class='emergency-alarm-static'>🚨 비상 상황 보고 중: [{st.session_state.issue_state}] 🚨</div>", unsafe_allow_html=True)

# [2. 통합 작업표준 확인]
st.markdown("<div class='section-title'>📋 작업 표준 및 품질 통합 확인</div>", unsafe_allow_html=True)
c_all = st.checkbox("✅ 작업표준및 작업지침 4대항목 확인 완료", key="confirm_v760")
...
for idx, row in enumerate(st.session_state.rows):
    ...
    dq = c[6].number_input("EA", min_value=0, key=f"dq_{rid}", label_visibility="collapsed")
...
if summary_list:
    ...
    total_t, total_a, total_d = df_sum['목표'].sum(), df_sum['실적'].sum(), df_sum['불량'].sum()
    
    line_achieve = round((total_a / total_t * 100), 1) if total_t > 0 else 0
    line_defect = round((total_d / total_a * 100), 1) if total_a > 0 else 0
    
    st.markdown(f"""
    <div class="total-stat-box">
        <span style="font-size:22px; color:#58a6ff;"><b>라인 전체 실적달성율: {line_achieve}%</b></span>
        <span style="font-size:22px; margin-left:40px; color:#ff7b72;"><b>라인 전체 실적불량률: {line_defect}%</b></span>
    </div>
    """, unsafe_allow_html=True)
    
    sh_cols = st.columns([1.5, 0.6, 0.6, 0.6, 1.5])
    for col, h in zip(sh_cols, ["기종명", "목표", "실적", "불량", "기종별 LOT"]): col.markdown(f"**{h}**")
...
# [7. 구글 시트 데이터 전송 엔진]
if st.button("🚀 데이터 최종 전송 및 연동 (v7.6.0)", type="primary", use_container_width=True):
    try:
        ...
        for r in st.session_state.rows:
            ...
            if p != "선택":
                final_data.append({
                    ...
                    "Material_Check": bom_str, "Issue_Status": "[CRITICAL]" if st.session_state.issue_state else "NORMAL",
        ...
        if final_data:
            ...
            df_new = pd.concat([df_old, pd.DataFrame(final_data)], ignore_index=True)
            conn.update(worksheet="sheet1", data=df_new)
            st.success("✅ [버전 7.6.0] 데이터가 무결하게 전송되었습니다."); st.balloons()
